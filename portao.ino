...
void loop()
{
 ...
 if (req.indexOf(F("/gpio/0")) != -1 && digitalRead(sensorMagnetico2) == LOW)
  ...
  client.print(F("</div>"));
  client.print(F("</div>"));
  client.print(F("<div class='row' style='margin-top: 35%; margin-bottom: 35%'>"));
  client.print(F("<a href='/gpio/0' class='row mt-5 mb-2 justify-content-center' style='text-decoration: none'>"));
  client.print(F("<button type='button' class='col-11 p-3 btn btn-outline-primary'><i class='bi bi-play'></i> Aperte</button>"));
  client.print(F("</a>"));
  client.print(F("<p class='text-center'>Aperte o botão para abrir ou fechar o portão</p>"));
  client.print(F("</div>"));
  client.print(F("</div>"));
  client.print(F("<footer class='fixed-bottom'>"));
  client.print(F("<p class='text-center'>"));
  client.print(F("<a href='https://www.linkedin.com/in/tiago-silva-pereira-84375750/'><i class='bi bi-linkedin'></i> Tiago </a> | Portão 2.0</p>"));
